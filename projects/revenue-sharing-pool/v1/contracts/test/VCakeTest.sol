...
contract VCakeTest {
    ...
    function _checkpoint(address _address, LockedBalance memory _prevLocked, LockedBalance memory _newLocked) internal {
        ...
        uint256 MAX_LOCK = CakePool.MAX_LOCK_DURATION();
        ...
        if (initialization[_user]) {
            ...
            if (cache.shares > 0) {
                if (cache.locked) {
                    ...
                    uint256 currentAmount = ((cache.cakePoolAvaliable + totalBoostDebt) * (cache.shares)) /
                        totalShares -
                        cache.userBoostedShare;
                    ...
                    if (
                        ...
                        // token.safeTransfer(treasury, currentOverdueFee); need to reduce cake balance in cake pool
                        cache.cakePoolAvaliable -= currentOverdueFee;
                    ...
                    // uint256 pool = CakePool.balanceOf();
                    uint256 currentShares;
                    if (totalShares != 0) {
                        currentShares =
                            (currentAmount * totalShares) /
                            ((cache.cakePoolAvaliable + totalBoostDebt) - currentAmount);
                    } else {
                    ...
                    uint256 totalAmount = (cache.shares * (cache.cakePoolAvaliable + totalBoostDebt)) / totalShares;
                    totalShares -= cache.shares;
                    cache.shares = 0;
                    ...
                    if (currentPerformanceFee > 0) {
                        // token.safeTransfer(treasury, currentPerformanceFee); need to reduce cake balance in cake pool
                        cache.cakePoolAvaliable -= currentPerformanceFee;
                    ...
                    if (totalShares != 0) {
                        newShares =
                            ...
                            ((cache.cakePoolAvaliable + totalBoostDebt) - totalAmount);
                    } else {
            ...
            //     // cache.cakePoolAvaliable += _amount;
            //     userCurrentAmount = _amount;
            // }

            // Calculate lock funds
            if (cache.shares > 0 && cache.locked) {
                userCurrentLockedBalance = ((cache.cakePoolAvaliable + totalBoostDebt) * cache.shares) / totalShares;
            ...
            //         ((cache.cakePoolAvaliable + totalBoostDebt) - userCurrentLockedBalance);
            // } else {
            ...
            if (cache.lockEndTime > cache.lockStartTime) {
                ...
                // uint256 boostWeight = ((cache.lockEndTime - cache.lockStartTime) * CakePool.BOOST_WEIGHT()) /
                //     DURATION_FACTOR;
            ...
            // user.cache.cakeAtLastUserAction = (user.cache.shares * balanceOf()) / totalShares - user.cache.userBoostedShare;
